...
export const DraggableObject = (props: any) => {
  ...
  const updateObjectByIndex = useStore(
    (state: any) => state.updateObjectByIndex
  );
  ...
  const currentObjectindex = objectArray.findIndex(
  ...
  const bind = useDrag(
    ({ active, movement: [x, y], timeStamp, event }: any) => {
      ...
      if (active && dragInfo.type === "object") {
        ...
        const newPos = {
          x: event.altKey
            ? Math.round(planeIntersectPoint.x)
            : planeIntersectPoint.x,
          y: object.pos[1],
          z: event.altKey
            ? Math.round(planeIntersectPoint.z)
  ...
  const onClickHandler = (e: any) => {
    ...
    if (dragInfo.isDragging && dragInfo.type === "line") {
      ...
      const objectIndex = objectArray.findIndex(
        (item: any) => item.id === object.id
      );
  ...
  const convexGeo = useMemo(() => {
    ...
    const verticles = object.points.map(
      (point: any) => new THREE.Vector3(point[0], point[1], point[2])
    );
  ...
  return (
    <>
      <mesh position={object.pos} {...(bind() as any)} onClick={onClickHandler}>
        <mesh>
      ...
      {object.type === "area" ? (
        <mesh position={object.pos}>
          {object.selected
            ? object.points.map((point: any, index: number) => (
                <AreaPointSphere
