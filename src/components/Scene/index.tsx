...
export const Scene = () => {
  ...
  const selectedObjectArray = objectArray.filter(
    (object: any) => object.selected
  );
  const unSelectedObjectArray = objectArray.filter(
    (object: any) => !object.selected
  );
  ...
  return (
    ...
    >
      ...
      <ambientLight color={ambientLightProps.color} />

      <directionalLight
        color={directionalLightProps.color}
        intensity={directionalLightProps.intensity}
        position={[
          directionalLightProps.position.x,
          directionalLightProps.position.y,
          directionalLightProps.position.z,
        ]}
      />
      ...
      <OrbitControls
        ...
        target={[
          orbitControlProps.target[0],
          orbitControlProps.target[1],
          orbitControlProps.target[2],
        ]}
      ...
      {/* {lineArray.map((line: any, index: number) => (
        <LineComponent key={`line${index}`} line={line} index={index} />
      ))} */}
